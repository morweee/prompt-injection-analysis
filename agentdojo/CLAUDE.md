...
- All elements implement `BasePipelineElement` with a single abstract method:
  ```python
  def query(query, runtime, env, messages, extra_args) -> tuple[str, FunctionsRuntime, Env, Sequence[ChatMessage], dict]
  ```
...
- Tool functions use `Annotated[..., Depends("attr_name")]` to inject state from the `TaskEnvironment` without exposing it to the LLM
...
Suites are versioned (`v1/`, `v1_1/`, ..., `v1_2_2/`). Each version directory contains only the suites and tasks that changed in that version. `TaskSuite.get_version_compatible_items` selects the highest compatible version for a given benchmark version. When adding or modifying tasks, add them to the appropriate version subdirectory.

Each suite contains user tasks (legitimate operations) and injection tasks (attack scenarios).

### Task Authoring Pattern

Every task subclasses `BaseUserTask[Env]` or `BaseInjectionTask[Env]` and must implement two abstract methods:

- `ground_truth(pre_environment) -> list[FunctionCall]`: returns the sequence of tool calls that correctly solve the task, used to validate and run the ground-truth agent.
- `utility(model_output, pre_environment, post_environment, strict) -> bool`: checks whether the agent completed the task correctly by comparing environment state before/after execution or inspecting model output.

The environment (`TaskEnvironment`) is a Pydantic `BaseModel` holding the mutable state the agent operates on (inbox, calendar, files, etc.). Use `deepdiff` comparisons between `pre_environment` and `post_environment` to verify state changes in `utility`.
...
2. **Tool Registration**: Functions are registered as tools with automatic schema generation from type hints and docstrings. Use `Annotated[Type, Depends("env_attr")]` to inject environment state into function parameters without exposing them to the LLM.
